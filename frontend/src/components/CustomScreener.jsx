...
import { listCustomScreens, createCustomScreen, deleteCustomScreen, runCustomScreen } from '../services/api';
...
export default function CustomScreener() {
    ...
    const handleAddCriterion = () => {
        setFormData({
            ...formData,
            criteria: [...formData.criteria, { field: 'roe', operator: '>', value: '' }]
        });
    };
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            setView('list');
            fetchScreens();
            setFormData({ name: '', description: '', logic: 'AND', criteria: [{ field: 'roe', operator: '>', value: '' }] });
        } catch (error) {
    ...
    if (view === 'results') {
        return (
            <div className="space-y-4">
                <button onClick={() => setView('list')} className="text-primary-600 hover:underline mb-4">
                    &larr; Back to Custom Screens
                </button>
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">Results: {activeScreenName}</h2>
                {loading ? (
        ...
        return (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-bold dark:text-gray-100">Create New Screen</h2>
                ...
                <form onSubmit={handleSave} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Logic</label>
                            <select
                    ...
                    <div className="space-y-4">
                        <h3 className="font-medium text-gray-900 dark:text-gray-200">Criteria</h3>
                        {formData.criteria.map((criterion, index) => (
                            <div key={index} className="flex flex-wrap md:flex-nowrap gap-4 items-end bg-gray-50 dark:bg-gray-750 p-3 rounded">
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Custom Screens</h2>
                    <p className="text-gray-600 dark:text-gray-400">Build and run your own screening strategies</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 gap-4">
                {screens.map((screen) => (
                    <div key={screen.name} className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md flex justify-between items-center">
                        <div>
                            <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{screen.name}</h3>
                            ...
                            <div className="mt-2 text-xs text-gray-500">
                                ...
                                Criteria: {typeof screen.criteria === 'string' ? JSON.parse(screen.criteria).length : Object.keys(screen.criteria || {}).length} conditions
                            </div>
                        </div>
                ...
                {screens.length === 0 && (
                    <div className="text-center py-10 bg-white dark:bg-gray-800 rounded-lg shadow">
                        <p className="text-gray-500 dark:text-gray-400">No custom screens found.</p>
                        <button onClick={() => setView('create')} className="text-primary-600 hover:underline mt-2">
