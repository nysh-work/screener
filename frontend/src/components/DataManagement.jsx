...
export default function DataManagement() {
  ...
  const handleUpdate = async (universe) => {
    if (
      !confirm(
        `This will fetch data for all ${universe} stocks. It may take a while. Continue?`
      )
    )
      return;
  ...
  const handleManualStockSubmit = async (e) => {
    ...
    try {
      ...
      setStatus({
        message: `Stock ${manualStock.ticker} added successfully!`,
        success: 1,
        errors: 0,
      });
      ...
      console.error(error);
      setStatus({
        error: 'Failed to add stock. Please check if it already exists.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleRefreshIndexData = async () => {
    if (
      !confirm(
        'This will refresh all index data (NIFTY 50, Midcap, Bank, IT, Auto). Continue?'
      )
    )
      return;
    ...
    <div className="max-w-4xl mx-auto py-8">
      <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-8">
          Update your local database with fresh data from Yahoo Finance. This
          process fetches the latest prices, fundamentals, and calculates
          metrics.
        </p>
        ...
        <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-8">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Refresh index data for NIFTY 50, Midcap, NIFTY Bank, NIFTY IT, and
              NIFTY Auto indices. This updates current prices, technical
              indicators, and market metrics.
            </p>
            <button
        ...
        <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Add individual stocks to your database
              </p>
              ...
              {!showManualForm ? (
                ...
                <form onSubmit={handleManualStockSubmit} className="space-y-3">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setManualStock({ ...manualStock, name: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-100"
                    required
                  />
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6">
              ...
              <p className="text-sm text-gray-500 mb-4">
                Add multiple stocks from CSV file
              </p>
        ...
        {status && (
          <div
            className={`mt-8 p-4 rounded-lg flex items-start ${status.error ? 'bg-red-50 text-red-800' : 'bg-green-50 text-green-800'}`}
          >
            {status.error ? (
              <AlertCircle className="w-5 h-5 mr-2 mt-0.5" />
            ) : (
              <RefreshCw className="w-5 h-5 mr-2 mt-0.5" />
            )}
            <div>
              {status.error ? (
                <p>{status.error}</p>
                ...
                <>
                  ...
                  <p className="text-sm mt-1">
                    Successfully updated: {status.success} stocks. Errors:{' '}
                    {status.errors}.
                  </p>
                </>
              )}
