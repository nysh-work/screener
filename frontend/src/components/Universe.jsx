...
export default function Universe() {
  ...
  const fetchUniverseData = async () => {
    try {
      ...
      setError(
        `Failed to load universe data: ${error.message}. Please try again.`
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredAndSortedData = universeData
    .filter((stock) => {
      ...
      if (
        ...
        stock.market_cap < parseFloat(filters.minMarketCap)
      )
        return false;
      if (
        filters.maxMarketCap &&
        stock.market_cap > parseFloat(filters.maxMarketCap)
      )
        return false;
      if (filters.minPE && stock.pe_ratio < parseFloat(filters.minPE))
        return false;
      if (filters.maxPE && stock.pe_ratio > parseFloat(filters.maxPE))
        return false;
      if (filters.minPB && stock.pb_ratio < parseFloat(filters.minPB))
        return false;
      if (filters.maxPB && stock.pb_ratio > parseFloat(filters.maxPB))
        return false;
      if (filters.minROE && stock.roe < parseFloat(filters.minROE))
        return false;
      if (filters.maxROE && stock.roe > parseFloat(filters.maxROE))
        return false;
      ...
      <div className="max-w-7xl mx-auto py-8">
        <div className="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded-lg p-6">
          ...
          <p className="text-red-700 dark:text-red-300 text-sm mb-4">
            We're having trouble connecting to the server. The backend API might
            be temporarily unavailable or experiencing issues.
          </p>
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Comprehensive data for all stocks in the database (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={filters.minPE}
                    onChange={(e) =>
                      setFilters({ ...filters, minPE: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={filters.maxPE}
                    onChange={(e) =>
                      setFilters({ ...filters, maxPE: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={filters.minPB}
                    onChange={(e) =>
                      setFilters({ ...filters, minPB: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={filters.maxPB}
                    onChange={(e) =>
                      setFilters({ ...filters, maxPB: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFilters({ ...filters, minROE: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFilters({ ...filters, maxROE: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-gray-600 dark:border-gray-500"
                  />
                </div>
                ...
                {columns.map((column) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      {column.sortable && (
                        <ArrowUpDown
                          className={`w-4 h-4 ml-1 ${sortField === column.key ? 'text-primary-600' : 'text-gray-400'}`}
                        />
                      )}
                    </div>
                  </th>
