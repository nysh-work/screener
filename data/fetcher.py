...
class YFinanceFetcher:
    ...
    def fetch_stock_info(self, ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            data = {
                ...
                "market_cap": clean_numeric(info.get("marketCap", 0)) / 1e7
                if info.get("marketCap")
                else None,  # Convert to Crores
                "isin": info.get("isin"),
    ...
    def fetch_fundamentals(self, ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            fundamentals = {
                ...
                "enterprise_value": clean_numeric(info.get("enterpriseValue", 0)) / 1e7
            ...
            if not balance_sheet.empty:
                try:
                    ...
                    fundamentals["total_assets"] = (
                        clean_numeric(latest_bs.get("Total Assets", 0)) / 1e7
                    )
                    ...
                    fundamentals["total_debt"] = (
                        clean_numeric(latest_bs.get("Total Debt", 0)) / 1e7
                    )
                    fundamentals["current_assets"] = (
                        clean_numeric(latest_bs.get("Current Assets", 0)) / 1e7
                    )
                    fundamentals["current_liabilities"] = (
                        clean_numeric(latest_bs.get("Current Liabilities", 0)) / 1e7
                    )
                except Exception as e:
            ...
            if not income_stmt.empty:
                try:
                    ...
                    fundamentals["revenue"] = (
                        clean_numeric(latest_is.get("Total Revenue", 0)) / 1e7
                    )
                    fundamentals["operating_profit"] = (
                        clean_numeric(latest_is.get("Operating Income", 0)) / 1e7
                    )
                    fundamentals["ebitda"] = (
                        clean_numeric(latest_is.get("EBITDA", 0)) / 1e7
                    )
                    fundamentals["net_profit"] = (
                        clean_numeric(latest_is.get("Net Income", 0)) / 1e7
                    )
                    fundamentals["interest_expense"] = (
                        clean_numeric(latest_is.get("Interest Expense", 0)) / 1e7
                    )
                except Exception as e:
            ...
            if not cashflow.empty:
                try:
                    ...
                    fundamentals["cash_flow_operations"] = (
                        clean_numeric(latest_cf.get("Operating Cash Flow", 0)) / 1e7
                    )
                except Exception as e:
    ...
    def calculate_derived_metrics(self, fundamentals: Dict, info: Dict = None) -> Dict:
        ...
        if fundamentals.get("enterprise_value") and fundamentals.get("ebitda"):
            metrics["ev_ebitda"] = calculate_ev_ebitda(
                fundamentals["enterprise_value"], fundamentals["ebitda"]
            )

        # ROE
        if fundamentals.get("net_profit") and fundamentals.get("total_equity"):
            metrics["roe"] = calculate_roe(
                fundamentals["net_profit"], fundamentals["total_equity"]
            )

        # ROCE (using EBITDA as proxy for EBIT)
        if (
            fundamentals.get("ebitda")
            and fundamentals.get("total_assets")
            and fundamentals.get("current_liabilities")
        ):
            capital_employed = (
                fundamentals["total_assets"] - fundamentals["current_liabilities"]
            )
            metrics["roce"] = calculate_roce(fundamentals["ebitda"], capital_employed)

        # Debt to Equity
        if fundamentals.get("total_debt") is not None and fundamentals.get(
            "total_equity"
        ):
        ...
        # Current Ratio
        if fundamentals.get("current_assets") and fundamentals.get(
            "current_liabilities"
        ):
    ...
    def calculate_quality_metrics(self, fundamentals: Dict, info: Dict = None) -> Dict:
        ...
        if info:
            metrics["promoter_holding"] = (
                clean_numeric(info.get("heldPercentInsiders", 0)) * 100
            )

        # OCF to Net Profit
        if (
            fundamentals.get("cash_flow_operations")
            and fundamentals.get("net_profit")
            and fundamentals["net_profit"] != 0
        ):
            metrics["ocf_to_net_profit"] = (
                fundamentals["cash_flow_operations"] / fundamentals["net_profit"]
            )
        ...
        if all(
            ...
            retained_earnings = 0
            total_liabilities = fundamentals.get("total_debt", 0) + fundamentals.get(
    ...
    def fetch_historical_financials(self, ticker: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(historical_data['revenue'])} historical data points for {ticker}"
            )
    ...
    def calculate_growth_metrics_from_history(
        self, ticker: str, historical_data: Dict
    ) -> Dict:
        """
        ...
        try:
            ...
            if len(net_profit) >= 4:
                metrics["profit_cagr_3y"] = calculate_cagr(
                    net_profit[-1], net_profit[-4], 3
                )

            if len(net_profit) >= 6:
                metrics["profit_cagr_5y"] = calculate_cagr(
                    net_profit[-1], net_profit[-6], 5
                )
    ...
    def calculate_technical_indicators_from_history(
        self, ticker: str, price_data: pd.DataFrame
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if len(close) >= 50:
                ...
                indicators["ema_20"] = float(ema_20.iloc[-1]) if not pd.isna(ema_20.iloc[-1]) else None
                indicators["ema_50"] = float(ema_50.iloc[-1]) if not pd.isna(ema_50.iloc[-1]) else None
            ...
            if len(close) >= 14:
                ...
                indicators["choppiness_index"] = float(ci.iloc[-1]) if not pd.isna(ci.iloc[-1]) else None
    ...
    def fetch_all_data(
        self, ticker: str, include_history: bool = True
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if include_history:
                ...
                if historical_financials:
                    growth_metrics = self.calculate_growth_metrics_from_history(
                        ticker, historical_financials
                    )
            ...
            if include_history:
                ...
                if price_data is not None:
                    technical_indicators = (
                        self.calculate_technical_indicators_from_history(
                            ticker, price_data
                        )
                    )
...
def get_nifty_500() -> List[str]:
    ...
    Returns:
        List of ticker symbols
    """
    # Return stocks from NIFTY 500
    nifty_500 = [
         "360ONE", "3MINDIA", "ABB", "ACMESOLAR", "AIAENG",
    "APLAPOLLO", "AWL", "AADHARHFC", "AARTIIND", "AAVAS",
    "ACE", "ADANIENSOL", "ADANIPOWER", "ATGL", "ABCAPITAL",
    "ABFRL", "ABLBL", "ABREL", "ABSLAMC", "AEGISLOG",
    "AEGISVOPAK", "AFCONS", "AFFLE", "AJANTPHARM", "AKUMS",
    "AKZOINDIA", "APLLTD", "ALKEM", "ALKYLAMINE", "ALOKINDS",
    "ARE&M", "AMBER", "ANANDRATHI", "ANANTRAJ", "ANGELONE",
    "APARINDS", "APTUS", "ASAHIINDIA", "ASTERDM", "ASTRAZEN",
    "ATHERENERG", "AIIL", "DMART", "BASF", "BEML",
    "BLS", "BSE", "BAJAJHLDNG", "BAJAJHFL", "BALKRISIND",
    "BALRAMCHIN", "BANKINDIA", "MAHABANK", "BATAINDIA", "BAYERCROP",
    "BDL", "BHARATFORG", "BHEL", "BHARTIHEXA", "BIKAJI",
    "BIOCON", "BSOFT", "BLUEDART", "BLUEJET", "BLUESTARCO",
    "BBTC", "FIRSTCRY", "BRIGADE", "MAPMYINDIA", "CCL",
    "CESC", "CGPOWER", "CRISIL", "CSBBANK", "COCHINSHIP",
    "COROMANDEL", "CREATIVE", "CROMPTON", "CUB", "CUMMINSIND",
    "CYIENT", "DCAL", "DCM", "DCMSRIND", "DATAPATTNS",
    "DEEPAKFERT", "DEEPAKNTR", "DELHIVERY", "DEVYANI", "DHANI",
    "DBL", "DISHTV", "DIXON", "LALPATHLAB", "DREDGECORP",
    "EIDPARRY", "EIH", "EPL", "ESABINDIA", "ESCORTS",
    "EXIDEIND", "FACT", "FAIRCHEM", "FEDERALBNK", "FIEMIND",
    "FINEORG", "FORTIS", "FRETAIL", "GABRIEL", "GAEL",
    "GANDHAR", "GATI", "GAYAPROJ", "GEPIL", "GICRE",
    "GILLETTE", "GLAND", "GLAXO", "GLS", "GMBREW",
    "GMMPFAUDLR", "GMMF", "GNFC", "GODFRYPHLP", "GODREJAGRO",
    "GODREJIND", "GODREJPROP", "GRANULES", "GRINDWELL", "GESHIP",
    "GULFOILLUB", "HATSUN", "HBLPOWER", "HDFC", "HEG",
    "HEIDELBERG", "HINDCOPPER", "HINDPETRO", "HINDZINC", "HONAUT",
    "HUDCO", "ICDSLTD", "ICRA", "IDBI", "IDFC",
    "IEX", "IFBAGRO", "IFBIND", "IFCI", "IIFL",
    "IIFLWAM", "INDIACEM", "INDIAMART", "INDIANB", "INDOCO",
    "INDOSTAR", "INDOTHAI", "INDRAPRASTHA", "INFIBEAM", "INFOBEANS",
    "INGERRAND", "INOXLEISUR", "INOXWIND", "INTELLECT", "IOB",
    "IPCALAB", "IRB", "IRCON", "ISEC", "ITDCEM",
    "ITI", "IVC", "JBCHEPHARM", "JKCEMENT", "JBMA",
    "JKTYRE", "JMFINANCIL", "JSWENERGY", "JSWINFRA", "JPPOWER",
    "J&KBANK", "JINDALSAW", "JSL", "JIOFIN", "JUBLINGREA",
    "JUBLPHARMA", "JWL", "JYOTHYLAB", "JYOTICNC", "KPRMILL",
    "KEI", "KPITTECH", "KSB", "KAJARIACER", "KPIL",
    "KALYANKJIL", "KARURVYSYA", "KAYNES", "KEC", "KFINTECH",
    "KIRLOSBROS", "KIRLOSENG", "KIMS", "LTF", "LTTS",
    "LTFOODS", "LATENTVIEW", "LAURUSLABS", "THELEELA", "LEMONTREE",
    "LICI", "LINDEINDIA", "LLOYDSME", "LODHA", "MMTC",
    "MRF", "MGL", "MAHSCOOTER", "MAHSEAMLES", "M&MFIN",
    "MANAPPURAM", "MRPL", "MANKIND", "MFSL", "MAXHEALTH",
    "MAZDOCK", "METROPOLIS", "MINDACORP", "MSUMI", "MOTILALOFS",
    "MPHASIS", "MCX", "NATCOPHARM", "NBCC", "NCC",
    "NHPC", "NLCINDIA", "NSLNISP", "NTPCGREEN", "NH",
    "NATIONALUM", "NAVA", "NAVINFLUOR", "NETWEB", "NEULANDLAB",
    "NEWGEN", "NAM-INDIA", "NIVABUPA", "NUVAMA", "OBEROIRLTY",
    "OFSS", "ORIENTCEM", "ORIENTELEC", "ORIENTPPR", "PAGEIND",
    "PARAGMILK", "PATANJALI", "PAYTM", "PFC", "PFIZER",
    "PHOENIXLTD", "PIIND", "PNB", "POLICYBZR", "POLYMED",
    "POLYPLEX", "PRAJIND", "PRICOLLTD", "PRINCEPIPE", "PRSMJOHNSN",
    "PTC", "PVRINOX", "QUESS", "RADICO", "RAINBOW",
    "RAJESHEXPO", "RALLIS", "RAMCOCEM", "RATNAMANI", "RBLBANK",
    "RECLTD", "RELAXO", "ROSSARI", "RTNPOWER", "RUCHI",
    "RVNL", "SAGCEM", "SANOFI", "SAREGAMA", "SBICARD",
    "SHARDACROP", "SCHAEFFLER", "SFL", "SHILPAMED", "SHREECEM",
    "SHRIRAMFIN", "SHYAMMETL", "SIEMENS", "SOBHA", "SOLARA",
    "SOLARINDS", "SONACOMS", "SPAL", "SPICEJET", "SRF",
    "STARHEALTH", "STLTECH", "SUMICHEM", "SUNDARMFIN", "SUNDARMHLD",
    "SUNDRMFAST", "SUNTV", "SUPREMEIND", "SUVENPHAR", "SUZLON",
    "SWANENERGY", "SWARAJENG", "SYMPHONY", "SYNGENE", "TATACHEM",
    "TATACOFFEE", "TATACOMM", "TATAPOWER", "TATAINVEST", "TATATECH",
    "TEJASNET", "THERMAX", "TIMETECHNO", "TITAGARH", "TORNTPHARM",
    "TORNTPOWER", "TRENT", "TRIDENT", "TRIVENI", "TTKPRESTIG",
    "TTL", "TV18", "TVSMOTOR", "UCOBANK", "UNOMINDA",
    "VEDL", "VENUSPIPES", "VGUARD", "VIPIND", "VOLTAS",
    "VTL", "WABAG", "WELCORP", "WELSPUNLIV", "WESTLIFE",
    "WHIRLPOOL", "WOCKPHARMA", "YESBANK", "ZENSARTECH", "ZOMATO",
    "ZYDUSLIFE", "ECLERX", "TDPOWER", "SUPRAJIT", "WONDERLA",
    "KNRCON", "ELECTCAST",
    ]
    return nifty_500
